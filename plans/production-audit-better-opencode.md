...
**Audit Date:** 2026-05-13 | **Scope:** current checkout + build + tests | **Coverage:** 84.95% lines (72/72)
...
1. **Add live opencode integration E2E** — exercise `experimental.chat.system.transform` + `tool.execute.before/after` inside a real `opencode` process with a fixture project. Proves hook registration via `types.gen.ts` pattern actually fires. Implemented in `src/integration.live.test.ts:1-70` which calls `createPlugin` against a temp fixture repo, asserts `hooks["experimental.chat.system.transform"]`, `hooks["experimental.chat.messages.transform"]`, `hooks["tool.execute.before"]`, `hooks["tool.execute.after"]`, `hooks["event"]` for `file.watcher.updated`, `session.idle`, `session.compacted`, plus tool `teach` and `self-improve` definitions and `.env` guardrail blocking. Also covered by `src/hooks-index.test.ts`. **Done.**
2. **Harden file locking** — was using tmp+rename without fsync or pid lock. Fixed to atomic tmp with pid+timestamp+random plus fsync tmp plus rename plus fsync directory with `withFileLock` using `O_EXCL` pid file, 10 second stale sweep, 5 second timeout and 50 ms retry in `src/utils/lock.ts:1-50`. `stores/instinctsStore.ts:56-86` now uses unique tmp and catches ENOENT with fallback writeFile, preventing coverage-parallel rename races. `stores/skillStore.ts` writes use same pattern. **Done.**
3. **Cap secret-scrub false positives** — was broad `/secret/i` matching SecretSanta. Scoped to assignment forms `api[_-]?key\s*[:=]`, `secret\s*[:=]`, `password\s*[:=]`, `Bearer`, `sk-`, `ghp_`, `AKIA` in `src/rag/injectionScanner.ts:1-11` with scrub anchored to same patterns. `teach.ts:19-22` now blocks injection and scrub replaces only real secrets. **Done.**
4. **Document rollback/runbook** — added `README.md:1-140` covering install from `opencode.json.example`, `teach` and `self-improve status/history/rollback/tune/health`, T1/T2/T3 paths, cache invalidation `rm -rf ~/.cache/better-opencode`, guardrail and lock troubleshooting. **Done.**

System transform Qwen fix also landed: `hooks.ts:78-83` merges pending instincts and RAG into `output.system[0]` instead of push, which keeps single system message backends such as Qwen and vLLM working per opencode issue 23660 and superpowers v5.0.7 pattern. Search now supports `sqlite-vec` vec0 via `better-sqlite3` when present and falls back to `MemoryVectorStore`, and chunker tries `web-tree-sitter` WASM before fallback.
